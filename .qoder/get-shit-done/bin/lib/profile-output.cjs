...
function cmdWriteProfile(cwd, options, raw) {
  ...
  for (const dimKey of DIMENSION_KEYS) {
    ...
    if (conf === 'HIGH') {
      highCount++;
      if (dim.qoder_instruction) summaryLines.push(`- **${dimensionLabels[dimKey] || dimKey}:** ${dim.qoder_instruction} (HIGH)`);
    } else if (conf === 'MEDIUM') {
      mediumCount++;
      if (dim.qoder_instruction) summaryLines.push(`- **${dimensionLabels[dimKey] || dimKey}:** ${dim.qoder_instruction} (MEDIUM)`);
    } else if (conf === 'LOW') {
      lowCount++;
    }
  }
  ...
  for (const dimKey of DIMENSION_KEYS) {
    ...
    const instruction = dim.qoder_instruction || 'No strong preference detected. Ask the developer when this dimension is relevant.';
    ...
    template = template.replace(new RegExp(`\\{\\{${dimKey}\\.qoder_instruction\\}\\}`, 'g'), instruction);
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.qoder', 'get-shit-done', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  for (const dimKey of DIMENSION_KEYS) {
    ...
    const confidence = dim.confidence || 'UNSCORED';
    let instruction = dim.qoder_instruction;
    if (!instruction) {
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.qoder', 'commands', 'gsd', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.qoder', 'AGENTS.md');
