...
Key references:
- @.qoder/get-shit-done/references/ui-brand.md (display patterns)
- @.qoder/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.qoder/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".qoder/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".qoder/get-shit-done/USER-PROFILE.md" ".qoder/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .qoder/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .qoder/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .qoder/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.qoder/get-shit-done/references/user-profiling.md`
...
Reference: @.qoder/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .qoder/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .qoder/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .qoder/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .qoder/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global AGENTS.md" -- "Add profile to .qoder/AGENTS.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .qoder/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .qoder/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .qoder/commands/gsd/dev-preferences.md"

**For AGENTS.md profile section (if selected):**

```bash
node .qoder/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .qoder/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to .qoder/AGENTS.md"
...
```bash
BACKUP_PATH=".qoder/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .qoder/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .qoder/commands/gsd/dev-preferences.md
  ✓ AGENTS.md section       ./AGENTS.md
  ✓ Global AGENTS.md        .qoder/AGENTS.md
```
