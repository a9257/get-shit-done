...
@.qoder/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
```
...
```bash
PHASE_INFO=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node ".qoder/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**Project skills:** Check .qoder/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
1. Read template: `.qoder/get-shit-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node ".qoder/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
**Project instructions:** Read ./AGENTS.md if exists — follow project-specific guidelines
**Project skills:** Check .qoder/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, plans should account for project skill rules
...
**Project skills:** Check .qoder/skills/ or .agents/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
3. **Clean up stale task directories:**
   ...
   Remove-Item -Recurse -Force "$env:USERPROFILE\.qoder\tasks\*" -ErrorAction SilentlyContinue
   ```
