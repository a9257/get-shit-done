...
```bash
PHASE_INFO=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".qoder/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
